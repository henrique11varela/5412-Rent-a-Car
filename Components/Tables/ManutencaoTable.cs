...
namespace Rent_a_Car.Components.Tables
{
    ...
    internal class ManutencaoTable : DataGridView
    {
        ...
        public ManutencaoTable(int margin_top, int margin_right, int margin_bottom, int margin_left) : this()
        {
        ...
        public void FillData(ArrayList list, int days = 0)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                this.Rows.Add(convertedItem.IdVeiculo, matricula, convertedItem.TipoVeiculo, convertedItem.DataInicio.Date.ToShortDateString(), convertedItem.DataPrevistaFim.Date.ToShortDateString(), convertedItem.Problema, "Detalhes");
            }
        }
    }
}
